...
public class GenAnimationTypes {
	...
	void OutputAnimationBaseTest (TextWriter tw)
	{
		tw.WriteLine (@"
[TestFixture]
public class {0}AnimationBaseTest
{{
	class {0}AnimationBasePoker : {0}AnimationBase
	{{
		protected override {1} GetCurrentValueCore ({1} defaultOriginValue, {1} defaultDestinationValue,
	...
	{{
		{0}AnimationBasePoker poker = new {0}AnimationBasePoker ();
		Assert.AreEqual (typeof ({1}), poker.TargetPropertyType);
	}}
}}
", type, gettype(type));
	}
...
", type, gettype(type));
	...
	void OutputTestHeader (TextWriter tw)
	{
		tw.WriteLine (@"
...
");
	...
	void OutputAnimationTest (TextWriter tw)
	{
		tw.WriteLine (@"
[TestFixture]
public class {0}AnimationTest
{{
	[Test]
	public void Properties ()
	{{
		Assert.AreEqual (""By"", {0}Animation.ByProperty.Name, ""1"");
		Assert.AreEqual (typeof ({0}Animation), {0}Animation.ByProperty.OwnerType, ""2"");
		Assert.AreEqual (typeof ({1}?), {0}Animation.ByProperty.PropertyType, ""3"");

		Assert.AreEqual (""From"", {0}Animation.FromProperty.Name, ""4"");
		Assert.AreEqual (typeof ({0}Animation), {0}Animation.FromProperty.OwnerType, ""5"");
		Assert.AreEqual (typeof ({1}?), {0}Animation.FromProperty.PropertyType, ""6"");

		Assert.AreEqual (""To"", {0}Animation.ToProperty.Name, ""7"");
		Assert.AreEqual (typeof ({0}Animation), {0}Animation.ToProperty.OwnerType, ""8"");
		Assert.AreEqual (typeof ({1}?), {0}Animation.ToProperty.PropertyType, ""9"");
	}}
}}
", type, gettype(type));
	}
...
", type, gettype(type));
	...
	void OutputKeyFrameTest (TextWriter tw)
	{
		tw.WriteLine (@"
[TestFixture]
public class {0}KeyFrameTest
{{
	[Test]
	public void Properties ()
	{{
		Assert.AreEqual (""KeyTime"", {0}KeyFrame.KeyTimeProperty.Name, ""1"");
		Assert.AreEqual (typeof ({0}KeyFrame), {0}KeyFrame.KeyTimeProperty.OwnerType, ""2"");
		Assert.AreEqual (typeof (KeyTime), {0}KeyFrame.KeyTimeProperty.PropertyType, ""3"");

		Assert.AreEqual (""Value"", {0}KeyFrame.ValueProperty.Name, ""4"");
		Assert.AreEqual (typeof ({0}KeyFrame), {0}KeyFrame.ValueProperty.OwnerType, ""5"");
		Assert.AreEqual (typeof ({1}), {0}KeyFrame.ValueProperty.PropertyType, ""6"");
	}}
}}
", type, gettype(type));
	}
...
", type, gettype(type));
	...
	void OutputTestFooter (TextWriter tw)
	{
		tw.WriteLine ("\n}"); /* close the namespace */
	}
...
", type, gettype(type), generateLerp ("baseValue", "value", "splineProgress"));
	...
	void OutputKeyFrameFile (KeyFrameType kt)
	{
		...
		string class_filename = String.Format ("System.Windows.Media.Animation/{0}{1}KeyFrame.cs", kt, type);
		string test_filename = String.Format ("Test/System.Windows.Media.Animation/{0}{1}KeyFrameTest.cs", kt, type);
		...
		case KeyFrameType.Discrete:
			OutputDiscreteKeyFrame (class_tw);
			break;
		case KeyFrameType.Linear:
			OutputLinearKeyFrame (class_tw);
			break;
		case KeyFrameType.Spline:
			OutputSplineKeyFrame (class_tw);
			break;
		}
